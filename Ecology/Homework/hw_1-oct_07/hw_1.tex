...
\begin{enumerate}[\ \ (a)]
    \item
        We see $R'(N) = \dfrac{a^2}{(H + aN)^2}$, which is never zero since $a > 0$, so we must look asymptotically to find the maximum value.  $R' \rightarrow 0$ as $N \rightarrow \infty$.  So $\displaystyle\lim_{N\rightarrow\infty}R(N) = r$.  This means that the maximum per-capita growth rate is $r$ but the population never attains this value since it was attained asymptotically.  The population is doomed to extinction for all initial conditinos if $r<0$.
    \item
        Since $R$ is an increasing function of $N$, $R$ attains its minimum on the boundary $N=0$.  This minimum is $R(0) = r - \dfrac{a}{H}$.
\end{enumerate}
